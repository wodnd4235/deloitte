...
package com.deloitte.solution.batch.support.dao;
...
JobInstanceDao, InitializingBean {
	...
	/**
	 * 2019-09-01
	 * @see com.deloitte.solution.batch.support.dao.JdbcJobInstanceDao#createJobInstance(String, JobParameters)
	 * quartz 스케쥴 아이디와 연동하기 위해 SCHED_HIST_ID, 컬럼 추가.
	 */
	private static final String CREATE_JOB_INSTANCE = "INSERT into %PREFIX%JOB_INSTANCE(JOB_INSTANCE_ID, SCHED_HIST_ID, JOB_NAME, JOB_KEY, VERSION)"
			+ " values (?, ?, ?, ?, ?)";
	...
	public JobInstance createJobInstance(String jobName,
		...
		// 2019-09-01 schedHistId 변수 정의
		String schedHistId = jobParameters.getString("schedHistId");
		...
		//2018-11-01 schedule id 검증 추가.
//		Assert.notNull(schedHistId, "schedId must not be null.");
//		Assert.notNull(schedulerHistoryId, "schedulerHistoryId must not be null.");
		...
		//2018-11-01 schedule id 추가.
		Object[] parameters = new Object[] { jobId, schedHistId, jobName,
				jobKeyGenerator.generateKey(jobParameters), jobInstance.getVersion() };
		getJdbcTemplate().update(
				getQuery(CREATE_JOB_INSTANCE),
				parameters,
				new int[] { Types.BIGINT, Types.BIGINT, Types.VARCHAR, Types.VARCHAR, Types.BIGINT });
	...
	public JobInstance getLastJobInstance(String jobName) {
		try {
			return getJdbcTemplate().queryForObject(getQuery(FIND_LAST_JOB_INSTANCE_BY_JOB_NAME),
					new Object[] { jobName, jobName }, new JobInstanceRowMapper());
		} catch (EmptyResultDataAccessException e) {
			return null;
		}
	}
